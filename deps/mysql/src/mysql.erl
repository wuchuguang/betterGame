...
-define(RECONNECT_INTERVAL, 5 * 1000).
...
-define(STATE_VAR, {?MODULE, mysql_connection_state}).
-define(CONNECT_TIMEOUT, 5000000).
...
start1(Server, PoolId, Host, Port, User, Password, Database, LogFun, Encoding,
    ...
    gen_server:StartFunc(
      {local, Server}, ?MODULE,
      [Server, PoolId, Host, Port, User, Password, Database, LogFun, Encoding], [{timeout, ?CONNECT_TIMEOUT}]).
...
%%
-spec connect(any(), PoolId::atom(), Host::string(), Port::integer() | undefined,
...
%%
-spec fetch(PoolId::atom(), Query::iolist()) -> query_result().
fetch(PoolId, Query) ->
    fetch(mysql_dispatcher, PoolId, Query, undefined).
...
%%
-spec prepare(any(), Name::atom(), Query::iolist()) -> ok.
...
%%
-spec unprepare(any(), Name::atom()) -> ok.
...
execute(Name, Params) when erlang:is_atom(Name), erlang:is_list(Params) ->
    case erlang:get(?STATE_VAR) of
	undefined ->
	    {error, not_in_transaction};
	State ->
...
execute(PoolId, Name) when erlang:is_atom(PoolId), erlang:is_atom(Name) ->
    execute(PoolId, Name, []).

execute(PoolId, Name, Timeout) when erlang:is_integer(Timeout) ->
    execute(PoolId, Name, [], Timeout);

execute(PoolId, Name, Params) when erlang:is_list(Params) ->
...
transaction(Server, PoolId, Fun, Timeout) ->
	...
	State ->
		...
		PoolId ->
		    case catch Fun() of
			error = Err -> erlang:throw(Err);
			{error, _} = Err -> erlang:throw(Err);
			{'EXIT', _} = Err -> erlang:throw(Err);
			Other -> {atomic, Other}
		    end;
		_Other ->
		    call_server(Server, {transaction, PoolId, Fun}, Timeout)
...
handle_call(quit, _From, State) ->
	erlang:self() ! close,
    Pools = State#state.pids_pools,
    [erlang:self() ! {'DOWN', ok, process, Pid, narmal} || {Pid, _} <- gb_trees:to_list(Pools)] ,
    {reply, ok, State};
...
encode(Val) ->
    encode(Val, false).
-spec encode(Val::term(), AsBinary::bool()) -> string() | binary() | {error, any()}.
encode(Val, false) when Val == undefined; Val == null ->
    "null";
encode(Val, true) when Val == undefined; Val == null ->
    <<"null">>;
encode(Val, false) when erlang:is_binary(Val) ->
	erlang:binary_to_list(quote(Val));
encode(Val, true) when erlang:is_binary(Val) ->
    quote(Val);
encode(Val, true) ->
	erlang:list_to_binary(encode(Val,false));
encode(Val, false) when erlang:is_atom(Val) ->
    quote(erlang:atom_to_list(Val));
encode(Val, false) when erlang:is_list(Val) ->
    quote(Val);
encode(Val, false) when erlang:is_integer(Val) ->
	erlang:integer_to_list(Val);
encode(Val, false) when erlang:is_float(Val) ->
...
two_digits(Nums) when erlang:is_list(Nums) ->
    [two_digits(Num) || Num <- Nums];
two_digits(Num) ->
    [Str] = io_lib:format("~b", [Num]),
    case erlang:length(Str) of
	1 -> [$0 | Str];
	_ -> Str
    end.
...
%%  MySQL query.
quote(String) when erlang:is_list(String) ->
    [39 | lists:reverse([39 | quote(String, [])])];	%% 39 is $'
quote(Bin) when erlang:is_binary(Bin) ->
	erlang:list_to_binary(quote(erlang:binary_to_list(Bin))).
