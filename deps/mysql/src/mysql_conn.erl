...
-define(DEFAULT_STANDALONE_TIMEOUT, 1000000).
...
post_start(Pid, LogFun) ->
	receive
		...
		{mysql_conn, Pid, {error, Reason}} ->
			?Log2(LogFun, error, "Pid(~p) receive error:~p", [Pid, Reason]),
			{error, Reason}
%% 	Unknown ->
%% 	    ?Log2(LogFun, error,
%% 		 "received unknown signal, exiting: ~p Pid: ~p", [Unknown, Pid]),
%% 	    {error, "unknown signal received"}
	after ?DEFAULT_STANDALONE_TIMEOUT ->
		?Log2(LogFun, error, "Pid(~p) receive timeout", [Pid]),
		{error, "timed out"}
	end.
							...
							if
								is_atom(Encoding) ->
									...
									do_query(Sock, RecvPid, LogFun,
											 ...
											 Version);
								is_list(Encoding) ->
									EncodingBinary = list_to_binary(Encoding),
									do_query(Sock, RecvPid, LogFun,
											 <<"set names '", EncodingBinary/binary, "'">>,
											 Version);
								true ->
...
%% 	Query1 = list_to_binary(Query),
	Query1 = unicode:characters_to_binary(Query, utf8),
		...
		{ok, Packet, InitSeqNum} ->
			...
			case AuthRes of
				...
				{ok, <<255:8, Code:16/little, Message/binary>>, _RecvNum} ->
					?Log2(LogFun, error, "init error ~p: ~p user ~p password ~p",
						  [Code, binary_to_list(Message), User, Password]),
