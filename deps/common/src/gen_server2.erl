...
%% -----------------------------------------------------------------
call(Name, Request) ->
    case is_self(Name) of
        true ->
            io:format("gen_server call self ~p ~p~n",[Name, exit(call_sefl)]),
            ok;
        false->
...
is_self(Name) when is_atom(Name) ->
    erlang:whereis(Name) =:= erlang:self();
is_self(PID)  when is_pid(PID)   ->
    PID =:= erlang:self();
is_self(_) ->
    false.

-define(GET_STATE, {process, state}).
get_state(Name) ->
    call(Name, ?GET_STATE).
...
init_it(Starter, Parent, Name0, Mod, Args, Options) ->
    ...
    case catch Mod:init(Args) of
        {ok, State} ->
            ...
            NewState = update_state(Mod, init, State),
            loop(Parent, Name, NewState, Mod, infinity, Debug);
        {ok, State, Timeout} ->
            proc_lib:init_ack(Starter, {ok, self()}),
            NewState = update_state(Mod, init, State),
            loop(Parent, Name, NewState, Mod, Timeout, Debug);
        {stop, Reason} ->
...
try_dispatch(Mod, Func, Msg, State) ->
    try
        if
            is_function(Msg,0) -> Msg(), {ok,{noreply,State}};
            is_function(Msg,1) -> Msg(State), {ok, {noreply,State}};
            true ->
...
try_handle_call(Mod, Msg, From, State) ->
    try
        if
            is_function(Msg,0) -> {ok,{reply,Msg(), State}};
            is_function(Msg,1) -> {Reply,NewState} = Msg(State), {ok,{reply, Reply, NewState}};
            Msg == ?GET_STATE  -> {ok,{reply,State, State}};
            true ->
...
try_terminate({_,Mod}, Reason, State) ->
...
handle_msg({'$gen_call', From, Msg}, Parent, Name, State, Mod) ->
    ...
    case Result of
        {ok, {reply, Reply, NState}} ->
            NewState = update_state(Mod, State, NState),
            reply(From, Reply),
            loop(Parent, Name, NewState, Mod, infinity, []);
        {ok, {reply, Reply, NState, Time1}} ->
            NewState = update_state(Mod, State, NState),
            reply(From, Reply),
            loop(Parent, Name, NewState, Mod, Time1, []);
        {ok, {noreply, NState}} ->
            NewState = update_state(Mod, State, NState),
            loop(Parent, Name, NewState, Mod, infinity, []);
...
handle_msg({'$gen_call', From, Msg}, Parent, Name, State, Mod, Debug) ->
    ...
    case Result of
        {ok, {reply, Reply, NState}} ->
            NewState = update_state(Mod, State, NState),
            Debug1 = reply(Name, From, Reply, NewState, Debug),
            loop(Parent, Name, NewState, Mod, infinity, Debug1);
        {ok, {reply, Reply, NState, Time1}} ->
            NewState = update_state(Mod, State, NState),
            Debug1 = reply(Name, From, Reply, NewState, Debug),
            loop(Parent, Name, NewState, Mod, Time1, Debug1);
        {ok, {noreply, NState}} ->
            NewState = update_state(Mod, State, NState),
        ...
        _ ->
            case ExitReason of
                ...
                _ ->
                    ...
                    io:format("ExitReason ~p ~n", [ExitReason]),
                    exit(ExitReason)
            end
    end.
...
format_status(Opt, Mod, PDict, State) ->
    io:format("format_status ~p~n",[Mod]),
    DefStatus = case Opt of
                    terminate -> State;
                    _ -> [{data, [{"State", State}]}]
                end,
    RealMod =
    case erlang:function_exported(boot_mod, module, 1) of
        true ->
            boot_mod:module(Mod);
        false->
            Mod
    end,
    case erlang:function_exported(RealMod, format_status, 2) of
        true ->
            case catch RealMod:format_status(Opt, [PDict, State]) of
                {'EXIT', _} -> DefStatus;
                Else -> Else
            end;
        _ ->
...
%% external_stacktrace() ->
%%     get({?MODULE, external_stacktrace}).
%%
%% external_stacktrace(Stacktrace)->
%%     put({?MODULE, external_stacktrace}, Stacktrace).

update_state(_Module, OldState, OldState) ->
    OldState;
update_state({_,Module},  OldState, NewState) ->
    update_state(Module,  OldState, NewState);
update_state(Module,  OldState, NewState) ->
    case erlang:function_exported(Module, handle_state, 2) of
        true ->
            Module:handle_state(OldState, NewState);
        false->
            NewState
    end.
