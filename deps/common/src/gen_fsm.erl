...
-export([start/3, start/4,
         ...
         send_all_state_event/2, get_state/1,
...
-define(GET_STATE, {?MODULE, get_state}).

get_state(PID) ->
    sync_send_event(PID, ?GET_STATE).
...
handle_msg(Msg, Parent, Name, StateName, StateData, Mod, _Time) -> %No debug here
    ...
    case catch dispatch(Msg, Mod, StateName, StateData) of
        {next_state, NStateName, NStateData} ->
            loop(Parent, Name, NStateName, update_state(Mod,StateData, NStateData), Mod, infinity, []);
        {next_state, NStateName, NStateData, Time1} ->
            loop(Parent, Name, NStateName, update_state(Mod,StateData, NStateData), Mod, Time1, []);
...
handle_msg(Msg, Parent, Name, StateName, StateData, Mod, _Time, Debug) ->
    ...
    case catch dispatch(Msg, Mod, StateName, StateData) of
        {next_state, NStateName, NStateData} ->
            ...
            loop(Parent, Name, NStateName, update_state(Mod,StateData, NStateData), Mod, infinity, Debug1);
        {next_state, NStateName, NStateData, Time1} ->
            Debug1 = sys:handle_debug(Debug, fun print_event/3,
                                      {Name, NStateName}, return),
            loop(Parent, Name, NStateName, update_state(Mod,StateData, NStateData), Mod, Time1, Debug1);
        {reply, Reply, NStateName, NStateData} when From =/= undefined ->
...
dispatch({'$gen_sync_event', _From, ?GET_STATE}, _Mod, StateName, StateData) ->
